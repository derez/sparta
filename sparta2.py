...
'''
import logging
logging.basicConfig(format="[%(module)s-%(lineno)d] %(message)s", filename='sparta.log', 
    level=logging.DEBUG)
...
try:
    ...
    print("[-] Import failed. PyQt4 library not found. \nTry installing it with: apt install python-qt4 - {!r}".format(err))
    exit(1)
...
try:
    ...
    try:
        ...
        print("[-] Import failed. QtWebKit library not found. \nTry installing it with: apt install python-pyside.qtwebkit - {!r}".format(err))
        exit(1)
